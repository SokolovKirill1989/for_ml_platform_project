...
def prepare_dataset(store, batch,**kwargs):
    ...
    feature_service = store.get_feature_service(f)
...
if __name__ == "__main__":
    ...
    spark = SparkSession(sc).builder.appName("f_predict").enableHiveSupport().getOrCreate()
    df = f"select * from cdm.nvg_data_gr_agr_all_r2"

    enriched_df = prepare_dataset(store, df)
    import mlflow

    # Load model as a Spark UDF.
    loaded_model = mlflow.pyfunc.spark_udf(spark, model_uri="models:/breast_cancer/1")
    ...
    if spark._jsparkSession.catalog().tableExists('cdm', 'output_storage'):
        df.write.partitionBy("date_part").format("parquet").mode("append").insertInto(f"cdm.output_storage")
    else:
        df.write.partitionBy("date_part").format("parquet").mode("append").saveAsTable(f"cdm.output_storage")
    spark.stop()