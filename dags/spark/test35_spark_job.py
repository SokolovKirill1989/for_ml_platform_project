...
def prepare_dataset(store, batch,**kwargs):
    ...
    feature_service = store.get_feature_service(None)
...
if __name__ == "__main__":
    ...
    spark = SparkSession(sc).builder.appName("test35_predict").enableHiveSupport().getOrCreate()
    df = f"select * from cdm.nvg_cre_date"
